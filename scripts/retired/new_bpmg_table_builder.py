...
def readSimIds():
    """ Parses the output of a simbad id search so as to match Gaia ids to 2MASS

    Currently things are extremely hardcoded...
    Things are complicated by the fact that the two 'identifier' names
    are not separated by anything
