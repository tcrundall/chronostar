...
def run_fit(burnin, steps, nfixed, nfree, fixed_groups, init_free_groups,
    ...
    state = None
    init_ages = np.array(pos)[:,-1]
    plt.hist(init_ages, bins=100, label="orig")
    for i in range(nburnins):
        ...
        sampler.reset()
        # debugging age issue, ages seem to strongler prefer being
        #  almost integers
        ages = np.array(pos)[:,-1]
        plt.hist(ages, bins=100, label="After {} steps"\
            .format(burnin_steps_per_run))
        plt.legend(loc='best')
        plt.show()
        pdb.set_trace()
